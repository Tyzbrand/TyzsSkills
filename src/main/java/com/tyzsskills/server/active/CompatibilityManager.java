...
public class CompatibilityManager {
    ...
    public static void ProcessMigration(ServerPlayer player){
        ...
        int totalRefund = 0;
        totalRefund += CheckRefund(player, legacy, "second_life_lvl", 20);
        totalRefund += CheckRefund(player, legacy, "Villager_follow_lvl", 12);
        totalRefund += CheckRefund(player, legacy, "night_vision_lvl", 18);
        totalRefund += CheckRefund(player, legacy, "fire_resistance_lvl", 18);
        totalRefund += CheckRefund(player, legacy, "friendly_piglins", 12);
        totalRefund += CheckRefund(player, legacy, "comestible_rotten_flesh_lvl", 12);
        totalRefund += CheckRefund(player, legacy, "harvester_lvl", 14);
    ...
    private static int CheckRefund(ServerPlayer player, LegacyData data, String oldKey, int amount){
        double value = data.getOldValue(oldKey);
